...
import {
  Configuration,
  WalmartAdsReportsApi,
  WalmartAdsReportsApiFactory,
  WalmartCampaignApi,
  WalmartCampaignApiFactory,
} from '../walmart-communicator';
import { ConfigService } from '@nestjs/config';

@Injectable()
export class WalmartReportClient {
  constructor(private readonly configService: ConfigService) {}
  createClient(brandId: number): WalmartAdsReportsApi {
    let config: Configuration;
    if (brandId == this.configService.get('WALMART_SANDBOX_BRAND_ID')) {
    ...
    return <WalmartAdsReportsApi>WalmartAdsReportsApiFactory(config);
  }
}
@Injectable()
export class WalmartCampaignClient {
  constructor(private readonly configService: ConfigService) {}
